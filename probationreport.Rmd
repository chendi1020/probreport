...
htmlTable(output_data1[c(1:11,13,17,22,25,28, 32,33,35,36:39,42,45,50:52,55,57:58,60),], align="rrrr",
          rgroup=rgroup, n.rgroup=c(1,2,8,1,1,1,1,1), 
          rgroupCSSseparator="   ", 
          ...
          css.cell = rbind(rep("background: lightgrey; padding-left: 3.5em; padding-right: 3.5em;", times=ncol(output_data1)+1),
                           matrix("", ncol=ncol(output_data1)+1, nrow=nrow(output_data1[c(1:11,13,17,22,25,28,32,33,35,36:39,42,45,50:52,55,57:58,60),]))))
```
...
library(Gmisc)
#colnames(output_data1) <- c('Demographic Variable', colnames(output_data1))
htmlTable(output_data1[c(1:12,14,18,23,26,29),], align="lrrr",
          rgroup=rgroup, n.rgroup=c(1,2,9,1,1,1,1,1), 
          rgroupCSSseparator="   ", 
          ...
          css.cell = rbind(rep("background: lightgrey; padding-left: 3.5em; padding-right: 3.5em;", times=ncol(output_data1)+1),
                           matrix("", ncol=ncol(output_data1)+1, nrow=nrow(output_data1[c(1:12,14,18,23,26,29),]))))
```
<br/>
<br/>
<br/>
...
##### ASUS Action by college and Term
```{r, echo=FALSE}
#Fall
ASUSDF <- ASUSD %>% filter(substr(Term_Code,1,2)=='FS') %>% arrange(Term_Seq_Id) %>%  group_by(Short_Name, Term_Code) %>% 
        summarise(Total.ASUS= n(), N.of.Conflict=sum(conflictrenum))%>% mutate(Pct.Conflict= N.of.Conflict/Total.ASUS)
ASUSDF1 <- ASUSD %>% filter(substr(Term_Code,1,2)=='FS') %>% arrange(Term_Seq_Id)
ASUSDF1$Short_Name <- ' Overall'
ASUSDF1 <- ASUSDF1  %>%  group_by(Short_Name, Term_Code) %>% 
        summarise(Total.ASUS= n(), N.of.Conflict=sum(conflictrenum))%>% mutate(Pct.Conflict= N.of.Conflict/Total.ASUS)
ASUSDF <- rbind(ASUSDF1, ASUSDF)


library(reshape2)
ASUSDF <- melt(ASUSDF , id.vars = c('Short_Name','Term_Code'))
ASUSDF$value <- ifelse(ASUSDF$variable=='Pct.Conflict', sprintf("%1.1f%%",ASUSDF$value*100), ifelse(ASUSDF$value>1000, format(round(ASUSDF$value), big.mark = ","),
                                                                                                   round(ASUSDF$value) ))
ASUSDF <- dcast(ASUSDF, Short_Name + variable ~ Term_Code)
ASUSDF <- ASUSDF %>% mutate(c=1)%>% group_by(Short_Name) %>% mutate(seq=cumsum(c)) 
ASUSDF <- as.data.frame( ASUSDF) %>% mutate(Short_Name= ifelse(seq>1, NA, as.character( Short_Name) )) %>% select(-c(seq,c))
names(ASUSDF)[1:2] <- c('College', ' ')
htmlTable(ASUSDF,rnames=F,caption="ASUS Action by colleges and Fall Terms",align="llrr",
          col.rgroup = c("none","none", "#F7F7F7"),
          css.cell = rbind(rep("background: lightgrey; padding-left: 1.7em; padding-right: 1.7em;", times=ncol(ASUSDF)),
                           matrix("", ncol=ncol(ASUSDF), nrow=nrow(ASUSDF))))
```
...
```{r, echo=FALSE}
#Fall
ASUSDF <- ASUSD %>% filter(substr(Term_Code,1,2)=='SS') %>% arrange(Term_Seq_Id) %>%  group_by(Short_Name, Term_Code) %>% 
        summarise(Total.ASUS= n(), N.of.Conflict=sum(conflictrenum))%>% mutate(Pct.Conflict= N.of.Conflict/Total.ASUS)
ASUSDF1 <- ASUSD %>% filter(substr(Term_Code,1,2)=='SS') %>% arrange(Term_Seq_Id)
ASUSDF1$Short_Name <- ' Overall'
ASUSDF1 <- ASUSDF1  %>%  group_by(Short_Name, Term_Code) %>% 
        summarise(Total.ASUS= n(), N.of.Conflict=sum(conflictrenum))%>% mutate(Pct.Conflict= N.of.Conflict/Total.ASUS)
ASUSDF <- rbind(ASUSDF1, ASUSDF)
...
ASUSDF <- as.data.frame( ASUSDF) %>% mutate(Short_Name= ifelse(seq>1, NA, as.character( Short_Name) )) %>% select(-c(seq,c))
names(ASUSDF)[1:2] <- c('College', ' ')
htmlTable(ASUSDF,rnames=F,caption="ASUS Action by colleges and Spring Terms",align="llrr",
          col.rgroup = c("none","none", "#F7F7F7"),
          css.cell = rbind(rep("background: lightgrey; padding-left: 1.7em; padding-right: 1.7em;", times=ncol(ASUSDF)),
                           matrix("", ncol=ncol(ASUSDF), nrow=nrow(ASUSDF))))
```