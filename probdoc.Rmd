...
kable(tab,rnames=F,caption="Students by acdemic level", align="lrrr"
          )
...
library(pander)
kable(tab1,rnames=F,caption="Table1 - Distribution of ASUS action count- All cohorts included"
          )
```
...
kable(tab2,caption="Table2 - Graduation Outcome in relation to the distribution of ASUS Action Counts - 
...
kable(tab3,caption="Table3 - THe average amount of loan and institutional aid - All Cohorts included"  )
```
...
kable(tab4,caption="Table4 - THe average amount of loan and institutional aid - Limited to Cohorts with six-year grad rate (till 2010 cohort)"
...
kable(output_data1[c(1:11,13,17,22,25,28, 32,33,35,36:39,42,45,50:52,55,57:58,60),], align="rrrr",
...
library(pander)
#colnames(output_data1) <- c('Demographic Variable', colnames(output_data1))
kable(output_data1[c(1:12,14,18,23,26,29),], align="lrrr",
...
kable(ASUSDF,rnames=F,caption="ASUS Action by colleges and Fall Terms",align="llrr")
```
...
ASUSDF <- as.data.frame( ASUSDF) %>% mutate(Short_Name= ifelse(seq>1, NA, as.character( Short_Name) )) %>% select(-c(seq,c))
names(ASUSDF)[1:2] <- c('College', ' ')
kable(ASUSDF,rnames=F,caption="ASUS Action by colleges and Spring Terms",align="llrr")
```
